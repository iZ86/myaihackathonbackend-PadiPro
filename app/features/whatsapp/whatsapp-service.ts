...
export class ReplyService {
    ...
    private async post(
        payload: SendTextPayload | SendImagePayload | SendAudioPayload | SendVideoPayload,
...
export class WhatsappService {
    ...
    private async handleImage(msg: IImageMessage, user: UserData): Promise<void> {
        if (msg.mediaId && msg.url) {
            ...
            const handleImageResult: Result<string> = await mainService.handleImage(
                user.mobile_no,
                saveImage.mediaName,
            );
    ...
    private async handleVideo(msg: IVideoMessage, user: UserData): Promise<void> {
        if (msg.mediaId && msg.url) {
            ...
            const handleVideoResult: Result<string> = await mainService.handleVideo(
                user.mobile_no,
                savedVideo.mediaName,
            );
    ...
    private async handleLocation(msg: ILocationMessage, user: UserData): Promise<void> {
        ...
        const handleLocationResult: Result<UserData> = await mainService.handleLocation(
            user.mobile_no,
            msg.latitude,
            msg.longitude,
        );
