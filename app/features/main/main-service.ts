  ...
  public async handleImage(mobile_no: string, mediaName: string): Promise<Result<string>> {
    ...
    if (imageOutput.detections[0]?.disease === "NOT DETECTED") {
      ...
      if (
        ...
        return Result.succeed(
          ENUM_STATUS_CODES_SUCCESS.OK,
          "I’m not confident in identifying this condition based on my current knowledge. Please provide more details.",
          "handleImage success.",
        );
      } else {
  ...
  public async handleVideo(mobile_no: string, mediaName: string): Promise<Result<string>> {
    ...
    if (geminiImageResult.isFailure()) {
      ...
      if (
        ...
        return Result.fail(
          ENUM_STATUS_CODES_FAILURE.TOO_MANY_REQUESTS,
          "You are sending videos too frequently, please send again in 1 minute.",
        );
      }
      throw Error(
        `handleVideo error, gemini error code ${geminiImageResult.getStatusCode()}: ${geminiImageResult.getMessage()}`,
      );
    }
